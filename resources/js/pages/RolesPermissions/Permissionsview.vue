    ...
    <div class="permissions-container">
        ...
        <div class="page-header">
            <div>
                ...
                <p class="page-description">Définissez les permissions disponibles dans l'application</p>
            </div>
        ...
        <div class="table-container">
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="perm in permissions" :key="perm.id" class="table-row">
                            <td>
                                <div class="perm-cell">
                                    <div class="perm-icon">
                            ...
                            <td>
                                <span class="perm-label">{{ formatPermission(perm.name) }}</span>
                            </td>
                            <td>
                                <div class="date-cell">
                                    ...
                                    {{ formatDate(perm.created_at as string) }}
                                </div>
                            </td>
                            <td>
                                <div class="action-buttons">
                                    <button @click="openEditModal(perm)" class="action-btn edit" title="Modifier">
                                        <svg class="action-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z" />
                                        </svg>
                                    </button>
                                    <button @click="confirmDelete(perm)" class="action-btn delete" title="Supprimer">
                                        <svg class="action-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        ...
                        <tr v-if="!permissions.length && !loading">
                            <td colspan="4">
                                <div class="empty-state">
                                    ...
                                    <p class="empty-title">Aucune permission</p>
                                    <p class="empty-description">Cliquez sur "Nouvelle permission" pour en créer une</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div class="pagination-container" v-if="meta && meta.last_page > 1">
                ...
                <div class="pagination-info">
                    Page {{ meta.current_page }} sur {{ meta.last_page }} • {{ meta.total }} permission(s)
                </div>
            </div>
        </div>
        ...
        <ConfirmModal
            ...
            :message="`Êtes-vous sûr de vouloir supprimer « ${permToDelete?.name} » ?`"
            details="Cette permission sera retirée de tous les rôles qui l'utilisent."
...
import PermissionModal from "@/components/RolePermission/PermissionModal.vue";
...
const meta = ref<Paginated<Permission>["meta"] | null>(null);
...
const openCreateModal = () => { selectedPermission.value = null; showModal.value = true; };
const openEditModal = (perm: Permission) => { selectedPermission.value = perm; showModal.value = true; };
const closeModal = () => { showModal.value = false; selectedPermission.value = null; };

const handleSave = async (data: { name: string }) => {
    try {
        if (selectedPermission.value) {
            await permissionService.updatePermission(selectedPermission.value.id, data);
        } else {
...
const confirmDelete = (perm: Permission) => { permToDelete.value = perm; showDeleteConfirm.value = true; };
const cancelDelete = () => { showDeleteConfirm.value = false; permToDelete.value = null; };
...
.page-title { font-size: 1.35rem; font-weight: 700; color: #0f172a; margin: 0 0 0.2rem; }
.page-description { color: #64748b; margin: 0; font-size: 0.8rem; }

.btn-primary {
    background: linear-gradient(135deg, #0ea5e9, #0284c7); color: #fff; border: none;
    padding: 0.55rem 1.1rem; border-radius: 9px; font-weight: 600; font-size: 0.8rem;
    cursor: pointer; display: flex; align-items: center; gap: 0.4rem;
    box-shadow: 0 2px 6px rgba(14, 165, 233, 0.3); transition: all 0.2s;
}
...
.finput {
    width: 100%; padding: 0.5rem 0.65rem 0.5rem 2rem; border: 1px solid #e2e8f0;
    border-radius: 7px; font-size: 0.8rem; color: #0f172a; background: #fff; transition: all 0.2s;
}
.finput:focus { outline: none; border-color: #0ea5e9; box-shadow: 0 0 0 3px rgba(14,165,233,.1); }

.table-container { background: #fff; border-radius: 12px; overflow: hidden; border: 0.5px solid #e2e8f0; }
...
.table td { padding: 0.75rem 1rem; border-bottom: 0.5px solid #f1f5f9; font-size: 0.8rem; color: #334155; vertical-align: middle; }
.table-row { transition: background 0.15s; }
.table-row:hover td { background: #f8fafc; }
.table tbody tr:last-child td { border-bottom: none; }

.perm-cell { display: flex; align-items: center; gap: 0.6rem; }
.perm-icon {
...
.perm-code { font-family: monospace; font-size: 0.78rem; color: #0284c7; background: #f0f9ff; padding: 0.15rem 0.4rem; border-radius: 4px; }
.perm-label { font-size: 0.8rem; color: #374151; font-weight: 500; }
