...
async function handleLogin(): Promise<void> {
  ...
  try {
    ...
    errorMsg.value = e.response?.data?.message ?? 'Erreur de connexion.'
  } finally {
    loading.value = false
  }
}
...
.form-group input { padding: .6rem .75rem; border: 1px solid #cbd5e1; border-radius: 8px; font-size: .9rem; outline: none; transition: border .2s; }
.form-group input:focus { border-color: #38bdf8; }
.btn-primary   { width: 100%; padding: .7rem; background: #0ea5e9; color: #fff; border: none; border-radius: 8px; font-size: .95rem; font-weight: 600; cursor: pointer; margin-top: .5rem; transition: background .2s; }
.btn-primary:hover { background: #0284c7; }
.btn-primary:disabled { opacity: .6; cursor: not-allowed; }
.alert-error   { background: #fee2e2; color: #b91c1c; padding: .6rem .75rem; border-radius: 8px; margin-bottom: 1rem; font-size: .875rem; }
.auth-link     { text-align: center; margin-top: 1rem; font-size: .875rem; color: #64748b; }
.auth-link a   { color: #0ea5e9; text-decoration: none; font-weight: 600; }
</style>
