<template>
    <div v-if="partenaire">
        ...
        <div class="info-grid">
            <div class="info-card">
                ...
                <dl>
                    <div class="dl-row">
                        <dt>Adresse</dt>
                        <dd>{{ partenaire.adresse ?? '—' }}</dd>
                    </div>
                    <div class="dl-row">
                        <dt>CP / Ville</dt>
                        <dd>{{ partenaire.cp }} {{ partenaire.ville }}</dd>
                    </div>
                    <div class="dl-row">
                        <dt>Secteur</dt>
                        <dd>{{ partenaire.secteur_activite ?? '—' }}</dd>
                    </div>
                    <div class="dl-row">
                        <dt>Salariés</dt>
                        <dd>{{ partenaire.nbrs_salaries ?? '—' }}</dd>
                    </div>
                    <div class="dl-row">
                        <dt>CA</dt>
                        <dd>{{ partenaire.ca ? `${partenaire.ca} €` : '—' }}</dd>
                    </div>
                    <div class="dl-row">
                        <dt>SIRET</dt>
                        <dd>{{ partenaire.siret ?? '—' }}</dd>
                    </div>
                    <div class="dl-row">
                        <dt>Tél. 1</dt>
                        <dd>{{ partenaire.telephone_1 ?? '—' }}</dd>
                    </div>
                    <div class="dl-row">
                        <dt>Tél. 2</dt>
                        <dd>{{ partenaire.telephone_2 ?? '—' }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="info-card">
                ...
                <div class="statut-actions">
                    <button v-for="t in transitions" :key="t.value" class="btn-transition"
        ...
        <div class="section">
            ...
            <!-- Filtres contacts -->
            <div class="filters">
                <input v-model="contactFilters.search" class="input" placeholder="Rechercher..."
        ...
        <!-- Modals -->
        <ContactModal v-if="showContactModal" :contact="selectedContact" :partenaire-id="partenaire.id"
            :statut-options="contactStatutOptions" @close="showContactModal = false" @saved="onContactSaved" />

        <ConfirmDialog v-if="showConfirm" message="Supprimer ce contact ?" @confirm="doDeleteContact"
